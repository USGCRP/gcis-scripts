...
# This creates a generic publication for the references having reftypes which do not exist in our 
# data model. The generic publication is subequently associated with the reference.
use v5.14; use Gcis::Client; use Data::Dumper; use Getopt::Long; use Pod::Usage; use strict; 
binmode(STDOUT, ":utf8"); my %types = (
    marticle => 'Magazine Article',
    narticle => 'Newspaper Article',
    earticle => 'Electronic Article',
    cpaper => 'Conference Paper',
    cproc => 'Conference Proceedings',
    thesis => 'Thesis',
    film => 'Film or Broadcast', ); $| = 1; my $type = "cproc"; my $url = 
"https://data.gcis-dev-front.joss.ucar.edu"; my $max_update = 100; my $dry_run = 0; my $help = 0;
 my $result = GetOptions ("type=s" => \$type,
                    "url=s" => \$url,
                    "max_update=i" => \$max_update,
                    "dry_run" => \$dry_run,
                    'help|?' => \$help); pod2usage(-verbose => 2) if $help;
 if (!grep $type eq $_, keys %types) {
     say "invalid type : $type";
     pod2usage(-verbose => 2);
 }
say " type : $type, $types{$type}"; print " url $url\n"; say " max update : $max_update"; say " dry 
run" if $dry_run; my $all = "?all=1";
# my $all;
my $ref_search = "/reference.json$all"; my $search = $url.$ref_search; say " search : $search"; my 
$g = $dry_run ? Gcis::Client->new( url => $url)
                 : Gcis::Client->connect(url => $url);
my $refs = $g->get($ref_search); my $n = @$refs; say " n refs : $n"; my $n_update = 0; my $i=0; for 
(@$refs) {
